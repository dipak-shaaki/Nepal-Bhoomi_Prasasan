...
module.exports = { registerLand, approveLand, requestLandEdit, approveLandEdit ,sellLand, buyLand};// controllers/landController.js
const Land = require('../models/landModel');
const convertToSquareMeter = require('../utils/convertToSqMeters');
...
// @desc Cancel land sale
// @route PUT /api/land/cancel-sale/:id
// @access Private (Only Owner)
const cancelSale = async (req, res) => {
  try {
    ...
    await land.save();
    res.status(200).json({ message: 'Land sale cancelled successfully', land });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
};
...
const getAllLands = async (req, res) => {
  try {
    const lands = await Land.find().populate('owner', 'name email').populate('buyer', 'name email');
    res.status(200).json({ message: 'Lands retrieved successfully', lands });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
};
...
const getLandById = async (req, res) => {
  try {
    const land = await Land.findById(req.params.id).populate('owner', 'name email').populate('buyer', 'name email');
    if (!land) {
      return res.status(404).json({ message: 'Land not found' });
    }
    res.status(200).json({ message: 'Land retrieved successfully', land });
  } catch (error) {
    res.status(500).json({ message: 'Server error', error: error.message });
  }
};
